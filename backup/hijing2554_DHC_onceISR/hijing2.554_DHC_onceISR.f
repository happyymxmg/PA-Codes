c  VERSION 2.554    Guo-Yang   09/16/2015
c  QCD evolution of shadowing factor is improved.
c
c  VERSION 2.553    Gabor Papp  01/22/2015
c  PDFSET initialization is taken out from PARTON. This should be done
c  now once in main.
c  pdflib is substituted by LHAPDF. GRV98lo grid is generated for LHAPDF
c
...
C
	...
	SAVE  /LUDAT1/
c	DHC
	INTEGER Jout(300,300)
C   ISR recover
	COMMON/ISRGL/JPIG(300),KFJPIG(300,500),PXJPIG(300,500),
     &                PYJPIG(300,500),PZJPIG(300,500),PEJPIG(300,500),
     &                PMJPIG(300,500),JTIG(300),KFJTIG(300,500),
     &                PXJTIG(300,500),PYJTIG(300,500),PZJTIG(300,500),
     &                PEJTIG(300,500),PMJTIG(300,500)
	SAVE  /ISRGL/
	INTEGER FJPIG(300),FJTIG(300)
...
C
	...
	PHI=2.0*HIPR1(40)*RLU(0)
        PHI=0.0 !! Wei-Tian, fix the reaction plane
	BBX=BB*COS(PHI)
	BBY=BB*SIN(PHI)
	HINT1(19)=BB
	HINT1(20)=PHI
...
C	DHC
C	IF(NFP(JP,5).LE.1 .AND. NFT(JT,5).GT.1) THEN
C		NP=NP+1
C		N01=N01+1
C	ELSE IF(NFP(JP,5).GT.1 .AND. NFT(JT,5).LE.1) THEN
C		NT=NT+1
C		N10=N10+1
C	ELSE IF(NFP(JP,5).LE.1 .AND. NFT(JT,5).LE.1) THEN
C		NP=NP+1
C		NT=NT+1
C		N0=N0+1
C	ELSE IF(NFP(JP,5).GT.1 .AND. NFT(JT,5).GT.1) THEN
C		N11=N11+1
C	ENDIF
...
C
	DO 150 I_JET=1,NJET
c		write(*,*) "Hard:", JP,JT,I_JET
       CALL JETINI(JP,JT,0)
	   CALL HIJHRD(JP,JT,JOUT(JP,JT),JFLG,1)
...
C		******** jet with PT>HIPR1(11) will be quenched
 ...
 160	CONTINUE
C	DHC
C	CALL HIJSFT(JP,JT,JOUT,IERROR)
C	IF(IERROR.NE.0) THEN
C	   IF(IHPR2(10).NE.0) WRITE(6,*) 'error occured in HIJSFT'
C	   GO TO 50
C	ENDIF
C
C		********conduct soft scattering between JP and JT
	JATT=JATT+JOUT(JP,JT)
...
C	DHC
	DO 210 JP=1,IHNT2(1)
	DO 210 JT=1,IHNT2(3)
c*** 	add the last JPth Projetile ISR gluons in the nucleon-string
	IF(FJPIG(JP).EQ.0)THEN
	DO 186 J=1,JPIG(JP)
		KFPJ(JP,NPJ(JP)+J)=KFJPIG(JP,J)
		PJPX(JP,NPJ(JP)+J)=PXJPIG(JP,J)
		PJPY(JP,NPJ(JP)+J)=PYJPIG(JP,J)
		PJPZ(JP,NPJ(JP)+J)=PZJPIG(JP,J)
		PJPE(JP,NPJ(JP)+J)=PEJPIG(JP,J)
		PJPM(JP,NPJ(JP)+J)=PMJPIG(JP,J)
186	CONTINUE
	NPJ(JP)=NPJ(JP)+JPIG(JP)
	FJPIG(JP)=1
	ENDIF	
c*** 	add the last JTth Target ISR gluons in the nucleon-string	
	IF(FJTIG(JT).EQ.0)THEN
	DO 187 J=1,JTIG(JT)
		KFTJ(JT,NTJ(JT)+J)=KFJTIG(JT,J)
		PJTX(JT,NTJ(JT)+J)=PXJTIG(JT,J)
		PJTY(JT,NTJ(JT)+J)=PYJTIG(JT,J)
		PJTZ(JT,NTJ(JT)+J)=PZJTIG(JT,J)
		PJTE(JT,NTJ(JT)+J)=PEJTIG(JT,J)
		PJTM(JT,NTJ(JT)+J)=PMJTIG(JT,J)
187	CONTINUE
	NTJ(JT)=NTJ(JT)+JTIG(JT)
	FJTIG(JT)=1
	ENDIF
	
	IF(SCIP(JP,JT).EQ.-1.0) GO TO 210
	...
	ELSE IF(NFP(JP,5).GT.1 .AND. NFT(JT,5).GT.1) THEN
		N11=N11+1
	ENDIF 
	CALL HIJSFT(JP,JT,JOUT(JP,JT),IERROR)
	IF(IERROR.NE.0) THEN
...
C		*********must have enough energy to produce jets
	...
	MIST=0
C
c	We allow the sea quark can scatter more than once
...
c********************************
c		write(*,*) "pythia-1:", PP(JP,4),PT(JT,4),JPIG(JP),JTIG(JT)
c    Add back the ISR gluons in last hard collision in current hard collision
		IF(JPIG(JP) .GT. 0) THEN
		DO 131 I=1,JPIG(JP)
		 PXP=PXP+PXJPIG(JP,I)
		 PYP=PYP+PYJPIG(JP,I)
		 PZP=PZP+PZJPIG(JP,I)
...
131		CONTINUE		
		...
		PP(JP,3)=PZP
		PP(JP,4)=PEP
C*	reset JPth Projectile ISR gluons common block	
		KFJPIG(JP,:) = 0	
		PXJPIG(JP,:) = 0
		PYJPIG(JP,:) = 0
		PZJPIG(JP,:) = 0
...
132		CONTINUE	
		...
		PT(JT,3)=PZT
		PT(JT,4)=PET
C*	reset JTth Target ISR gluons common block	
		KFJTIG(JT,:) = 0	
		PXJTIG(JT,:) = 0
		PYJTIG(JT,:) = 0
		PZJTIG(JT,:) = 0
		...
		PMJTIG(JT,:) = 0
		ENDIF
c		write(*,*) "pythia-2:", PP(JP,4),PT(JT,4)
 155	CALL PYTHIA
	JJ=MINT(31)
...
C************************correction made on Oct. 14,1994*****
	   ENDIF
c      find the sea quark	   
c	   IF(K(5,2).NE.21.AND.K(3,2).EQ.21) IP_SEA = 1
c	   IF(K(6,2).NE.21.AND.K(4,2).EQ.21) IT_SEA = 1	 	   
	   IF(K(I,3).EQ.7.OR.K(I,3).EQ.3) THEN
	      IF(IP_SEA.EQ.0.AND.IS7.EQ.1.AND.K(5,2).NE.21) GOTO 155	   
	      IF(K(I,3).EQ.7.AND.K(I,2).NE.21.AND.K(I,2).EQ.K(7,2)
     &	             .AND.IS7.EQ.0.AND.IP_SEA.lt.1) THEN
		 PP(JP,10)=P(I,1)
		 PP(JP,11)=P(I,2)
		 PP(JP,12)=P(I,3)
		 PZP=PZP+P(I,3)   
     ...
     &                               I_INIRAD.EQ.0)) THEN
c***	find its sister		 
c		 IF(IP_SEA.EQ.1.AND.K(I,2).EQ.-K(7,2)) then
c	     IP_SEA=IP_SEA+1	
c		 GOTO 199	 
c		 ENDIF
		 PXP=PXP+P(I,1)
		 ...
		 PEP=PEP+P(I,4)
		 GO TO 180 
C* 		Gluons in Projectile ISR 				 
	      ELSE IF(K(I,3).EQ.3.AND.K(I,2).EQ.21) THEN	
		  NPIG=NPIG+1
		  KFJPIG(JP,NPIG)=K(I,2)
		  PXJPIG(JP,NPIG)=P(I,1)
		  PYJPIG(JP,NPIG)=P(I,2)
		  PZJPIG(JP,NPIG)=P(I,3)
		  PEJPIG(JP,NPIG)=P(I,4)
		  PMJPIG(JP,NPIG)=P(I,5)
		  GOTO 180	
	      ENDIF	  
199	      JPP=JPP+1
	      IP(JPP,1)=I
	   ...
	   ELSE IF(K(I,3).EQ.8.OR.K(I,3).EQ.4) THEN
	      IF(IT_SEA.EQ.0.AND.IS8.EQ.1.AND.K(6,2).NE.21) GOTO 155	   
	      IF(K(I,3).EQ.8.AND.K(I,2).NE.21.AND.K(I,2).EQ.K(8,2)
     &				.AND.IS8.EQ.0.AND.IT_SEA.lt.1) THEN
		 PT(JT,10)=P(I,1)
		 PT(JT,11)=P(I,2)
		 PT(JT,12)=P(I,3)
		 PZT=PZT+P(I,3)   
     ...
     &                             I_INIRAD.EQ.0)) THEN
c***	find its sister		 
c		 IF(IT_SEA.EQ.1.AND.K(I,2).EQ.-K(8,2)) then
c	     IT_SEA=IT_SEA+1	
c		 GOTO 299	 
c		 ENDIF	 
		 PXT=PXT+P(I,1)
		 ...
		 PET=PET+P(I,4)
		 GO TO 180
C* 		Gluons in Target ISR 				 
	      ELSE IF(K(I,3).EQ.4.AND.K(I,2).EQ.21) THEN	
c		write(*,*)"ISR",NTIG,I,K(I,2),P(I,4)
		  NTIG=NTIG+1
		  KFJTIG(JT,NTIG)=K(I,2)
		  PXJTIG(JT,NTIG)=P(I,1)
		  PYJTIG(JT,NTIG)=P(I,2)
		  PZJTIG(JT,NTIG)=P(I,3)
		  PEJTIG(JT,NTIG)=P(I,4)
		  PMJTIG(JT,NTIG)=P(I,5)
		  GOTO 180		 
	      ENDIF
299	      JTT=JTT+1
	      IT(JTT,1)=I
 ...
 180	CONTINUE
c
c	gluon number in ISR respectively
		JPIG(JP) = NPIG
		JTIG(JT) = NTIG
C
...
c       If(NFP(JP,10).EQ.-1 .and. NFT(JT,10).EQ.-1)goto 2111
c       If(NFP(JP,10).EQ.0 .and. NFT(JT,10).EQ.0)goto 2111
c         write(*,*)'before',PP(JP,4)+PT(JT,4)
         ETOT=PP(JP,4)+PT(JT,4)
         Etot0=Etot
C	DHC
         IF(NFP(JP,10).EQ.1)THEN  
c         IF(NFP(JP,10).NE.0)THEN 
...
********************************************************
	HINT1(11)=HINT1(14)-APHX1*HINT1(15)
     &			-APHX2*HINT1(16)+APHX1*APHX2*HINT1(17)
c	write(*,*) "cross sections:",HINT1(11),HINT1(14), 
c     & HINT1(15),HINT1(16),HINT1(17)
...
c*********************Q-shadowing*************************
	INTEGER III,JJJ,KKK
	INTEGER,PARAMETER :: NA1=200
	INTEGER,PARAMETER :: NA2=1000
	double precision, target :: Ta(NA1+NA2)
	double precision, target :: Tgf(NA1*NA2)
	double precision, target :: Tqf(NA1*NA2)
	double precision, target :: Tqu(NA1*NA2)
	double precision, target :: Tqub(NA1*NA2)
	double precision, target :: Tqd(NA1*NA2)
	double precision, target :: Tqdb(NA1*NA2)
	double precision, target :: Tqs(NA1*NA2)
	REAL*8 :: XXX(2),fgl(NA1,NA2),fqu(NA1,NA2),fqd(NA1,NA2)
     & ,fqs(NA1,NA2),fqub(NA1,NA2),fqdb(NA1,NA2)
	INTEGER :: NA(2)
	COMMON/shadow/ Ta,Tgf,Tqf,Tqu,Tqub,Tqd,Tqdb,Tqs
	SAVE  /shadow/  
	DATA Ta/1200*0.0/
c************************************* 2015.9.16 Guo-Yang Ma
...
C************************
c	We read the shadowing factor's table for further
c	interpolation in subroutine PARTON.
c	The key idea is to set a common target 2-D array.
c*********************************

	IF(Ta(1) .EQ. 0.0 .AND. Ta(100) .EQ. 0.0) then
	IF(IHNT2(1) .EQ. 197 .OR. IHNT2(3) .EQ. 197) THEN
	OPEN(211,FILE="./shadowing-HOPPET/R_AuAu_0.28.dat")
	ELSEIF(IHNT2(1) .EQ. 207 .OR. IHNT2(3) .EQ. 207) THEN	
	OPEN(211,FILE="./shadowing-HOPPET/R_PbPb_0.28.dat")
	ELSEIF(IHNT2(1) .EQ. 1 .OR. IHNT2(3) .EQ. 1) THEN
	goto 505
	ELSE
	WRITE(*,*) "Wrong flag to set nuclus type : Q-table"
	stop
	ENDIF	
	write(*,*) "reading shadowing grids"
!	Q-loop  (2nd-D)
	DO 333 IQ = 1, NA2 
!	  x-loop  (1st-D)
	  DO 222 Ix = 1, NA1
	read (211,*) x_frac,q_tem,R_sbar,R_dbar,R_ubar 
     & ,R_gluon,R_u,R_d,R_s
!	Scale (x)&(lnQ^2) dimensions
	If(IQ==1) Ta(Ix) = x_frac
	If(Ix==1) Ta(IQ+NA1) = q_tem
!	Store the shadowing factor for gluon&quark
	fgl(Ix,IQ) = R_gluon
	fqu(Ix,IQ) = R_u
	fqub(Ix,IQ) = R_ubar
	fqd(Ix,IQ) = R_d
 ...
 333	CONTINUE
	...
	Tqdb(:)=Pack(fqdb(:,:),.True.)
	Tqs(:)=Pack(fqs(:,:),.True.)
c	write(*,'(10(D12.5,1x))')Ta
c	We will fill up the table in SUBROUTINR PARTON
...
C === taken out by Pg 01/22/2015
      ...
      COMMON /SG1/ SG
C  *****************************************
      REAL RX(7),RQX,RGX,SG
	INTEGER,PARAMETER :: NA1=200
	INTEGER,PARAMETER :: NA2=1000
	REAL*8 :: XXX(2),fgl(NA1,NA2),fqu(NA1,NA2)
	INTEGER :: NA(2)
	double precision, target :: Ta(NA1+NA2)
	double precision, target :: Tgf(NA1*NA2)
	double precision, target :: Tqf(NA1*NA2)
	...
	double precision, target :: Tqdb(NA1*NA2)
	double precision, target :: Tqs(NA1*NA2)
	double precision, pointer :: FA(:), FG(:), FU(:)
     &  , FUB(:), FD(:), FDB(:), FS(:)
	COMMON/shadow/ Ta,Tgf,Tqf,Tqu,Tqub,Tqd,Tqdb,Tqs
	SAVE  /shadow/
	DATA NA/NA1,NA2/
...
C === taken out by Pg 01/22/2015
CC *******************************
C       COMMON/PV/PDFVALUE1,PDFVALUE2,PDFVALUE3
C       INTEGER PDFVALUE1,PDFVALUE2,PDFVALUE3
CC  *****************************************
C      CALL PDFVALUE
C      PARM(1)='NPTYPE'
C      PARM(2)='NGROUP'
...
C === taken out by Pg 01/22/2015
	...
	INTERFACE
          ...
          DOUBLE PRECISION,POINTER :: ENT(:), Table(:)
          INTEGER   NENT(NARG)
          dimension ARG(NARG)
          INTEGER   INDEX(32)
          dimension WEIGHT(32)
...
c        SG=0.28
c        SQ=0.1
c        IF(IHPR2(6).EQ.1 .AND. IHNT2(1).GT.1) THEN
c          A=DBLE(IHNT2(1))
c          AAX=1.193*LOG(A)**0.16666666
c           RQX=1.0+AAX*(X1**3-1.2*X1**2+0.21*X1)
c           RGX=1.0+AAX*(X1**3-1.2*X1**2+0.21*X1)
c           IF(IP_CRS.EQ.1 .OR.IP_CRS.EQ.3) THEN
c            RQX=SQ*(A**0.3333333-1.0)**0.6
c            RQX=RQX*(1.0-3.5*SQRT(X1))*EXP(-X1**2/0.01)
c            RGX=SG*(A**0.3333333-1.0)**0.6
c            RGX=RGX*(1.0-1.5*X1**0.35)*EXP(-X1**2/0.004)
c           ENDIF
c           F(1,7)=RGX*F(1,7)
c           DO 400 I=1,6
c            F(1,I)=RQX*F(1,I)
c 400       CONTINUE
c        ENDIF
c        IF(IHPR2(6).EQ.1 .AND. IHNT2(3).GT.1) THEN
c          A=DBLE(IHNT2(3))
c          AAX=1.193*LOG(A)**0.16666666
c           RQX=1.0+AAX*(X2**3-1.2*X2**2+0.21*X2)
c           RGX=1.0+AAX*(X2**3-1.2*X2**2+0.21*X2)
c          IF(IP_CRS.EQ.2 .OR.IP_CRS.EQ.3) THEN
c            RQX=SQ*(A**0.3333333-1.0)**0.6
c            RQX=RQX*(1.0-3.5*SQRT(X2))*EXP(-X2**2/0.01)
c            RGX=SG*(A**0.3333333-1.0)**0.6
c            RGX=RGX*(1.0-1.5*X2**0.35)*EXP(-X2**2/0.004)            
c           ELSE
c           ENDIF
c           F(1,7)=RGX*F(1,7)
c********** F(2,7)? S.M. H & G.Y. M
...
c        ENDIF
C*****************************************************************
c**********Replaced by G.Y Ma 2015.9.16
c
c**********Use cern_fint(interpolation) to get shadowing factor
c**********We consider R(x,Q) = R0(x) - Rs(x,Q)
c**********The b-dependece of R is added in the ordinary way
	SG=0.28
	SQ=0.1
        IF(IHPR2(6).EQ.1 .AND. IHNT2(1).GT.1) THEN
		XXX(1) = X1
...
c		XXX(1) = 0.01
c		XXX(2) = 1.5
c            RX(7)=FINT(2,XXX,NA,FA,FG)
c	write(*,*) "FINT:", XXX,RX(7)
c	stop
           A=DBLE(IHNT2(1))
           AAX=1.193*DLOG(A)**0.16666666
           ...
           IF(IP_CRS.EQ.1 .OR.IP_CRS.EQ.3) THEN
            RX(7)=RGX - FINT(2,XXX,NA,FA,FG)
            RX(1)=RQX - FINT(2,XXX,NA,FA,FU)
            RX(2)=RQX - FINT(2,XXX,NA,FA,FUB)
            RX(3)=RQX - FINT(2,XXX,NA,FA,FD)
            RX(4)=RQX - FINT(2,XXX,NA,FA,FDB)
            RX(5)=RQX - FINT(2,XXX,NA,FA,FS)
            RX(6)=RQX - FINT(2,XXX,NA,FA,FS)
	    IF(A .EQ. 2.0) THEN
...
c*************************** Shadowing factor interpolation
